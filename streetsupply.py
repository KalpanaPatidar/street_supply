...
# --------------- Search ---------------
    ...
    st.title("🔍 Search by Location and Product")
    location_input = st.text_input("📍 Enter your location (e.g., Bhawarkua, Indore)")
    keyword_input = st.text_input("🔎 Enter product name (e.g., Rice, Oil, Plate)")
    ...
    if location_input or keyword_input:
        ...
        if not filtered.empty:
            st.success(f"✅ Found {len(filtered)} matching products.")
            st.dataframe(filtered[['name', 'supplier', 'supplier_location', 'price', 'discount', 'rating', 'reviews']])
        else:
            st.warning("⚠️ No matching products found.")
...
# --------------- Cart ---------------
    ...
    pay_method = st.radio("💳 Choose Payment Method", ["Cash on Delivery", "Pay Online"])
    if st.button("✅ Place Order"):
        if pay_method == "Cash on Delivery":
            st.success("✅ Order Placed with COD. Delivery in progress.")
        else:
            order_data = {
            ...
            st.markdown("""
            <a href="https://rzp.io/l/YOUR_PAYMENT_LINK" target="_blank">
                <button style="background-color:#4CAF50;color:white;padding:10px;border:none;border-radius:5px;">💳 Pay with Razorpay</button>
            </a>
            """, unsafe_allow_html=True)
            st.success("✅ Payment link generated. Complete payment to confirm order.")
...
# --------------- Orders Page (Simulation) ---------------
elif menu == "Orders":
    st.info("📦 This is your order history. (Simulation only in MVP)")
    st.warning("Coming soon: Delivery tracking, cancellation, and reviews!")

# --------------- Review & Comparison Section ---------------
st.markdown("### 📊 Price and Reviews Comparison")
query = st.text_input("🔍 Compare Product (e.g., Oil, Plate, Flour)")
user_location = st.text_input("📍 Location to Compare")

if query:
    compare = products[products['name'].str.contains(query, case=False)]
    if user_location:
        compare = compare[compare['supplier_location'].str.contains(user_location, case=False)]
    compare = compare.sort_values(by=["price", "rating"])
    for _, row in compare.iterrows():
        st.markdown(f"""
        - *{row['name']}* from {row['supplier']}
        - Price: ₹{row['price']} | Discount: {row['discount']}%
