...
st.title("🛒 Bhojan Bazaar")
st.markdown("**India's Local Grocery Market Finder**")

# Navigation menu
menu_options = ["Home", "Search", "Cart", "Profile"]
...
# Home Section
if menu == "Home":
    st.header("Welcome to Bhojan Bazaar 👋")
    st.markdown("""
    Discover fresh and local groceries from suppliers near you.  
    Use the search feature to find what you need and compare prices and delivery times.
    """)

# Search Section
elif menu == "Search":
    st.header("🔎 Search for Groceries")
...
# Cart Section
    ...
    if not st.session_state.cart:
        ...
        cart_items = products[products['product_id'].isin(st.session_state.cart)]
        total_price = 0

        for _, row in cart_items.iterrows():
            discounted_price = row['price'] * (1 - row['discount'] / 100)
            total_price += discounted_price + row['delivery_charge']
...
# Profile Section
    ...
    name = st.text_input("Name", "Kalpana Patidar")
    email = st.text_input("Email", "kalpanapatidar01@gmail.com")
    address = st.text_area("Address", "Mandsaur, India")
