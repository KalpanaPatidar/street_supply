...
# Load Lottie animation (optional header banner)
with open("lottie/animation.json") as f:
    lottie_anim = json.load(f)
...
# --- Header Bar with Icons (Search + Account + Wishlist + Cart + Orders) ---
col1, col2, col3, col4, col5 = st.columns([5, 1, 1, 1, 1])
with col1:
    search = st.text_input("🔍 Search products", placeholder="Search onions, oil...")
with col2:
    if st.button("👤", help="Account"):
...
# Toggle filter after search
if search:
    st.toggle("📍 Nearest Location", key="location_filter")

# ------------------------ ACCOUNT ------------------------
if st.session_state.get("menu") == "Account":
    st.subheader("👤 Your Profile")
...
# ------------------------ HOME PAGE CATEGORIES ------------------------
    ...
    depts = products['category'].unique()
    for dept in depts:
        st.image("https://via.placeholder.com/300x150.png?text=" + dept.replace(" ", "+"), width=300)
        if st.button(dept):
            st.session_state.selected_category = dept

    if st.session_state.get("selected_category"):
        dept = st.session_state.selected_category
        st.markdown(f"### 🛒 {dept} Items")
        dept_items = products[products['category'] == dept]
        for _, row in dept_items.iterrows():
            st.markdown(f"**{row['name']}** (₹{row['price']} → ₹{row['price']*(1-row['discount']/100):.2f})")
            st.markdown(f"⭐ {row['rating']} | 📍 {row['supplier_location']} | 🚚 ₹{row['delivery_charge']}")
